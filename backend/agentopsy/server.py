...
`python -m agentopsy.server` runs the api standalone for debugging: once uvicorn has
...
the human at the terminal or a wrapper script):
    ...
    AGENTOPSY_API_READY {"url": "http://127.0.0.1:54321", "token": "..."}
...
from agentopsy.i18n import LANG_HEADER, LanguageMiddleware
from agentopsy.routers import (
    agent,
    artifacts,
    capabilities,
    cases,
...
from agentopsy.security import HostHeaderMiddleware, allowed_hosts, new_session_token
...
def create_app(port: int, ui_origins: Sequence[str] = ()) -> FastAPI:
    ...
    ``AGENTOPSY_UI_ORIGINS`` (nginx reenvía el Host original del navegador, así
    ...
    app.add_middleware(
        ...
        allow_headers=["X-Agentopsy-Token", LANG_HEADER, "Content-Type"],
...
class _ReadyAnnouncingServer(uvicorn.Server):
    ...
    async def _announce_when_ready(self) -> None:
        ...
        print("AGENTOPSY_API_READY " + json.dumps(self._ready_payload), flush=True)
