...
Vive aparte de `agentopsy.i18n` (el motor: resolución del idioma, `t`, `Mensaje`,
...
CATALOGO["api.timeoutInteger"] = {
    "en": "AGENTOPSY_EXECUTOR_TIMEOUT must be an integer number of seconds greater than 0",
    "es": "AGENTOPSY_EXECUTOR_TIMEOUT debe ser un entero de segundos > 0",
}
...
CATALOGO["evidence.inboxUndefined"] = {
    "en": "AGENTOPSY_EVIDENCE_DIR is not defined: there is no evidence inbox. In the compose it is set by the api service (/evidence, mounted from ./evidence of the repo). In standalone mode, export the variable pointing at your evidence folder.",
    "es": "AGENTOPSY_EVIDENCE_DIR no está definido: no hay bandeja de evidencias. En el compose la fija el servicio api (/evidence, montado desde ./evidence del repo). En modo standalone, exporta la variable apuntando a tu carpeta de evidencias.",
}
CATALOGO["evidence.inboxMissing"] = {
    "en": "AGENTOPSY_EVIDENCE_DIR points at {root}, which does not exist or is not a directory. Create the folder (./evidence in the repo, if you use the compose) and leave the images to register inside it.",
    "es": "AGENTOPSY_EVIDENCE_DIR apunta a {root}, que no existe o no es un directorio. Crea la carpeta (./evidence en el repo, si usas el compose) y deja dentro las imágenes a registrar.",
}
...
CATALOGO["claude.loginHint"] = {
    "en": "Sign in ONCE inside the container: `docker compose exec -it api claude auth login` (the session persists in the agentopsy-cli-auth volume; it is revoked with `docker compose down -v`). If you run the backend outside the compose, run `claude auth login` on that machine.",
    "es": "Inicia sesión UNA VEZ dentro del contenedor: `docker compose exec -it api claude auth login` (la sesión persiste en el volumen agentopsy-cli-auth; se revoca con `docker compose down -v`). Si ejecutas el backend fuera del compose, ejecuta `claude auth login` en esa máquina.",
}
CATALOGO["claude.expiredHint"] = {
    "en": "The Claude Code session stored in the agentopsy-cli-auth volume is no longer valid (expired or revoked). Renew it WITHOUT leaving the application: Settings, Executors / AI, expand Claude Code and press «Renew the Claude Code session»; the dialog gives you the URL to open and asks you to paste back the code the browser returns. Note: `claude auth status` still returns `loggedIn: true` with an expired token, so the executor may appear available until a run is attempted. If you prefer the terminal: {hint}",
    "es": "La sesión de Claude Code guardada en el volumen agentopsy-cli-auth ya no es válida (caducada o revocada). Renuévala SIN salir de la aplicación: Configuración, Ejecutores / IA, despliega Claude Code y pulsa «Renovar sesión de Claude Code»; el diálogo te da la URL que abrir y te pide pegar de vuelta el código que devuelve el navegador. Ojo: `claude auth status` sigue devolviendo `loggedIn: true` con un token caducado, así que el ejecutor puede aparecer como disponible hasta que se intenta una corrida. Si prefieres la terminal: {hint}",
}
...
CATALOGO["codex.loginHint"] = {
    "en": "Sign in ONCE inside the container: `docker compose exec -it api codex login --device-auth` (device-code flow for environments with no browser; if the CLI refuses it, enable it in the security settings of your ChatGPT account). The session persists in the agentopsy-cli-auth volume; it is revoked with `docker compose down -v`. If you run the backend outside the compose, run `codex login` on that machine.",
    "es": "Inicia sesión UNA VEZ dentro del contenedor: `docker compose exec -it api codex login --device-auth` (flujo device-code para entornos sin navegador; si el CLI lo rechaza, actívalo en los ajustes de seguridad de tu cuenta ChatGPT). La sesión persiste en el volumen agentopsy-cli-auth; se revoca con `docker compose down -v`. Si ejecutas el backend fuera del compose, ejecuta `codex login` en esa máquina.",
}
...
CATALOGO["maletin.urlUnset"] = {
    "en": "the api service does not have the toolkit exec-agent URL configured ('{env}'). In the compose it is set by the api service (http://{service}:8666); in a standalone run, export it.",
    "es": "el servicio api no tiene configurada la URL del exec-agent del maletín ('{env}'). En el compose la fija el servicio api (http://{service}:8666); en ejecución standalone expórtala.",
}
...
CATALOGO["agent.badMaxAttempts"] = {
    "en": "AGENTOPSY_MAX_TOOL_ATTEMPTS={raw} is not valid: it must be an integer greater than or equal to 1.",
    "es": "AGENTOPSY_MAX_TOOL_ATTEMPTS={raw} no es válido: debe ser un entero >= 1.",
}
...
CATALOGO["agentLoop.repairTailNoJson"] = {
    "en": "Your reply contained no JSON object at all, so nothing in it reached the examiner. If that text WAS your final answer, write it again IN FULL as {\"action\": \"final\", \"text\": \"<the answer in markdown>\"}: it has not been recorded anywhere and nothing is lost by repeating it. If instead you meant to run a tool, issue that \"tool_call\" or \"tool_batch\". Do not start new work that was not already pending, and put no text outside the JSON.",
    "es": "Tu respuesta no contenía ningún objeto JSON, así que nada de ella llegó al perito. Si ese texto ERA tu respuesta final, vuelve a escribirla COMPLETA como {\"action\": \"final\", \"text\": \"<la respuesta en markdown>\"}: no ha quedado registrada en ningún sitio y no se pierde nada por repetirla. Si en cambio querías ejecutar una herramienta, emite ese \"tool_call\" o \"tool_batch\". No inicies trabajo nuevo que no estuviera ya pendiente, y no pongas texto fuera del JSON.",
}
...
CATALOGO["agentLoop.exhausted"] = {
    "en": "The maximum number of iterations ({max}) was reached with no final answer. Check the runs in `~/.agentopsy/cases/{case}/artifacts/` to see what was executed.",
    "es": "Se alcanzó el máximo de iteraciones ({max}) sin respuesta final. Revisa los runs en `~/.agentopsy/cases/{case}/artifacts/` para ver lo ejecutado.",
}
...
# --- FUNCIÓN «VISTAS» del grafo del caso (en prueba, 2026-09-04) --------------
CATALOGO["graphv.unknownAxis"] = {
    "en": "the graph view {view} is not one this case can cut by. Valid axes: {valid}.",
    "es": "la vista de grafo {view} no es un corte que este caso admita. Ejes válidos: {valid}.",
}
