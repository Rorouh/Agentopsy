...
Auth: the session (``~/.codex/auth.json``) lives in the ``agentopsy-cli-auth``
...
def codex_home() -> Path:
    """``CODEX_HOME`` real del CLI (en el compose, el volumen agentopsy-cli-auth)."""
...
class CodexExecutor(CliPromptExecutor):
    ...
    def run(self, prompt: str, context: dict[str, Any] | None = None) -> ExecutorResult:
        ...
        fd, path = tempfile.mkstemp(prefix="agentopsy-codex-", suffix=".md")
        os.close(fd)
