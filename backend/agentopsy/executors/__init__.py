...
from agentopsy.i18n import Mensaje, t, traducir_excepcion
...
from agentopsy.executors.codex import (
    CodexExecutor,
    read_model_catalog,
    supported_efforts,
...
def executor_models(executor_id: str) -> dict[str, object]:
    ...
    - ``codex`` returns the REAL catalog too, plus the reasoning levels each
      ...
      INVARIANT 7). See ``agentopsy.executors.codex.read_model_catalog``.
...
def executors_status() -> dict[str, dict[str, object]]:
    ...
    is unavailable. Consumed by ``agentopsy.capabilities.snapshot``."""
