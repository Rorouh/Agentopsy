...
Thin adapter over ``agentopsy.cases.manager.case_manager`` and
``agentopsy.evidence.evidence_manager`` (CLAUDE.md RULE 3). All routes are token-gated
...
from agentopsy.pulse import case_pulse
...
def get_case_pulse(case_id: str) -> dict[str, Any]:
    ...
    la lógica vive en ``agentopsy.pulse`` (RULE 3)."""
    try:
...
def register_evidence_async(case_id: str, req: RegisterEvidenceRequest) -> dict[str, Any]:
    ...
    desacoplado (``agentopsy.evidence_jobs``) y se sondea con
