...
``agentopsy.graph`` (RULE 3): aquí no se valida un grafo ni se funde nada.
...
- ``POST …/graphs/extract`` lanza la extracción como JOB de fondo y devuelve el
  ...
  el primer rechazo (ver ``agentopsy.graph.lote``).
...
def case_graph(case_id: str, vista: str | None = None) -> dict[str, Any]:
    ...
    # parámetro de la firma y `agentopsy/graph/vistas.py`.
