...
- The physical home of every tool is one or both compose maletines: `toolkits`
  ...
  live in one. `agentopsy.toolkit.maletin` probes those services and is what
...
- The legacy `delivery` (per-host-OS `bundled` | `container`) + `container_image` fields
  ...
  execution onto the maletines is pending.
"""
...
from agentopsy.path_policy import PathParameter, PathRole
...
class Tool:
    ...
    # `agentopsy.toolkit.maletin` probes these; `capabilities` reports them. RULE 2:
