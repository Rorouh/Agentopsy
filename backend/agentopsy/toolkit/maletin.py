...
Resolver order (CLAUDE.md RULE 1): env override (`AGENTOPSY_<BIN>_BIN`, resolvable where
...
from agentopsy.toolkit.resolver import resolve
...
# per-service via AGENTOPSY_TOOLKIT_WINDOWS_CONTAINER / AGENTOPSY_TOOLKIT_UNIX_CONTAINER
...
_DEFAULT_CONTAINER = {
    TOOLKIT_WINDOWS: "agentopsy-toolkit-windows",
    TOOLKIT_UNIX: "agentopsy-toolkit-unix",
}
...
_URL_ENV = {
    TOOLKIT_WINDOWS: "AGENTOPSY_TOOLKIT_WINDOWS_URL",
    TOOLKIT_UNIX: "AGENTOPSY_TOOLKIT_UNIX_URL",
}
...
def container_name(service: str) -> str:
    ...
    key = f"AGENTOPSY_{service.upper().replace('-', '_')}_CONTAINER"
...
def _request(
    ...
    token = os.environ.get("AGENTOPSY_EXEC_AGENT_TOKEN")
    if token:
        headers["X-Agentopsy-Exec-Token"] = token
