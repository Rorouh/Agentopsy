...
def session_context_budget_chars() -> int:
    ...
    ``AGENTOPSY_SESSION_CONTEXT_MAX_CHARS``; an invalid value fails loud."""
    raw = os.environ.get("AGENTOPSY_SESSION_CONTEXT_MAX_CHARS")
    if raw is None:
        return 400_000
    try:
        value = int(raw)
    ...
    if value <= 0:
        raise RuntimeError(
            f"AGENTOPSY_SESSION_CONTEXT_MAX_CHARS={raw!r} no es válido: debe ser "
...
def keep_last_tool_results_default() -> int:
    ...
    ``AGENTOPSY_CONTEXT_KEEP_TOOL_RESULTS``. Minimum 1 (the model always sees at
    least the latest result in full)."""
    try:
        return max(1, int(os.environ.get("AGENTOPSY_CONTEXT_KEEP_TOOL_RESULTS", "4")))
    except ValueError:
        return 4
