...
Modes (``AGENTOPSY_REDACTION_MODE``):
...
def get_redaction_mode() -> RedactionMode:
    """Read AGENTOPSY_REDACTION_MODE from the env. Default ``strict``.

    Unknown values fail LOUD (RULE 2 — no fallback to "closest valid"). The
    server startup is the canonical place to surface a typo.
    """
    raw = os.environ.get("AGENTOPSY_REDACTION_MODE", "strict").strip().lower()
    if raw not in _VALID_MODES:
        raise ValueError(
            f"invalid AGENTOPSY_REDACTION_MODE={raw!r}; "
...
def apply_redaction(
    ...
    if mode == "off":
        result["redaction"] = {"applied": False, "reason": "AGENTOPSY_REDACTION_MODE=off"}
        return result
