...
We keep the schemas under ``agentopsy.mcp.schemas`` (a sibling of the
...
from agentopsy.artifact_ref import ArtifactRef
...
class YaraParams(_StrictModel):
    ...
    operator stages rule bundles under e.g. ``~/.agentopsy/cases/<case>/rules/``.
    """
...
class JqParams(_StrictModel):
    ...
    an artifact at ``~/.agentopsy/cases/<case>/artifacts/<run>/stdout.txt``
...
class Volatility3Params(_StrictModel):
    ...
    wrapper contract (see ``agentopsy.toolkit.wrappers.volatility3``). To
    ...
    plugin: Literal[_VOLATILITY_PLUGINS] = Field(  # type: ignore[valid-type]
        description=(
            "Volatility3 plugin name (module + class). This enum is AGENTOPSY's "
...
class ChainsawParams(_StrictModel):
    ...
    (SEC-1). Bundled rule sets go under e.g. ``~/.agentopsy/cases/<case>/rules/``.
    """
