...
connected client. Claude Desktop spawns one ``python -m agentopsy.mcp`` per
...
- ``agent_package``: loaded once on ``select_case`` from
  ``agentopsy.agent.registry.agent_registry``. Its ``policy/tools.yaml``
...
from agentopsy.cases.manager import OsProfileUnresolved, case_manager, resolve_os_profile
...
class McpSession:
    ...
    def __post_init__(self) -> None:
        """Honour AGENTOPSY_CASE_ID as a *test-only* preselection.
        ...
        """
        preset = os.environ.get("AGENTOPSY_CASE_ID", "").strip()
        if not preset:
            return
