...
- Delega cada call_tool en ``agentopsy.toolkit.dispatcher.execute`` con el
...
from agentopsy.mcp.jira_tools import (
...
from agentopsy.mcp.session import McpSession
from agentopsy.path_policy import PathPolicyError, inject_evidence_path
from agentopsy.toolkit.catalog import BY_ID

logger = logging.getLogger("agentopsy.mcp")
...
def build_server(consent_ref: str) -> tuple[Server, Lifecycle]:
    ...
    server: Server = Server("agentopsy-toolkit")
    ...
    async def _list_tools() -> list[types.Tool]:
        ...
        for tool_id in pkg.policy.allowed_tools:
            ...
            if params_schema is None:
                logger.warning(
                    "tool %s lacks a schema in agentopsy.mcp.schemas.SCHEMA_BY_TOOL; "
...
async def _dispatch_forensic(
    ...
    if tool is None:
        ...
        return _error(f"unknown tool: {name!r} (not in agentopsy.toolkit.catalog)")
    ...
    if params_schema is None:
        return _error(
            ...
            f"agentopsy.mcp.schemas.SCHEMA_BY_TOOL; refusing to invoke with raw "
    ...
    from agentopsy.mcp.redaction import apply_redaction
