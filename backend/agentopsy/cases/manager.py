...
from agentopsy.config import CONFIG_DIR
    ...
    def apply_detected_evidence(
        ...
        Transitions (source in parentheses):
          ...
          - a DIFFERENT confident family than the derived one → CONFLICT: clear
            ...
            auto-routing (both sub-agents at once) is out of scope: a case
            carries ONE profile, and a conflict is resolved by the operator.
        ...
        # first: clear it and escalate (RULE 2). Routing each evidence to its own
        # sub-agent, instead of forcing one profile per case, is not implemented.
