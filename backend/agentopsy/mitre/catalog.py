...
def _agents_dir() -> Path:
    """Misma resolución que `agentopsy.agent.registry._default_agents_dir`.

    Se reimplementa en vez de importarla: `agentopsy.findings.store` consume este
    módulo y `agentopsy.agent` consume el store, así que importar `agentopsy.agent`
    aquí cerraría un ciclo de imports.
    """
    env = os.environ.get("AGENTOPSY_AGENTS_DIR")
    if env:
        return Path(env).resolve()
    # agentopsy/mitre/catalog.py → agentopsy/mitre → agentopsy → backend → <repo>
...
def unavailable_reason() -> str | None:
    ...
    return (
        ...
        f"(AGENTOPSY_AGENTS_DIR) so the closed technique enum can be loaded."
    )
...
def enterprise_unavailable_reason() -> str | None:
    ...
    return (
        ...
        "with the api image under agentopsy/mitre/enterprise.json."
    )
