...
INDICE: tuple[SeccionIndice, ...] = (
    ...
    SeccionIndice(
        ...
        contrato=(
            "La sección más extensa: qué se ejecutó, sobre qué evidencia y con "
            "qué resultado, desde `trabajos` del material. Jerarquía con `h3`: "
            "un resumen de ejecuciones (tabla de `uso_de_tools`) y después un "
            "subapartado por evidencia con una entrada por ejecución. Por cada "
            "ejecución, un bloque `kv` (run_id, herramienta y versión, "
            "evidencia y SHA-256 baseline, inicio y fin en UTC, código de "
            "salida, SHA-256 de stdout/stderr, número de ficheros de salida, "
            "artefactos de entrada, hallazgos derivados) y un bloque `code` con "
            "el argv. El argv se copia CARÁCTER A CARÁCTER de "
            "`trabajos[].argv_literal`: es el comando literal auditado, no una "
            "reconstrucción ni la intención declarada por el modelo, y el "
            "validador rechaza el informe entero si un bloque `code` no coincide "
            "con un argv auditado. Las ejecuciones con código de salida distinto "
            "de 0 se imprimen IGUAL, con su error: un informe que solo muestra "
            "lo que funcionó no es reproducible. Cierra señalando que un tercero "
            "con la misma imagen, el mismo maletín y estos argv reproduce el "
            "análisis."
        ),
    ),
    SeccionIndice(
