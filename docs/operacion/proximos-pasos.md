...
| Guard rail de perfil en los prompts | ✅ Regla nueva en `system.md` de `forensia-unix` y `forensia-windows`: ante mismatch detectado, el agente se niega a invocar tools y pide a la operadora reabrir el caso con el perfil correcto. RULE 2 intacta: nunca auto-switch. **⚠ Superseded por el enrutado automático (Fase 2 backend hecha); migración de prompts pendiente (pase dedicado) y UI en Fase 3.** |
