...
| Guard rail de perfil en los prompts | ✅ Regla en `system.md`/`playbook.md` de `forensia-unix` y `forensia-windows`: ante mismatch detectado, el agente se niega a invocar tools y pide a la operadora **anclar** el perfil correcto (el re-enrutado al sub-agente correcto es automático tras el anclaje, Fase 2 backend). RULE 2 intacta: nunca auto-switch. **Migración de prompts al flujo de anclaje HECHA (Fase 4b, este commit); UI en Fase 3.** |
| Banner de desajuste en UI | ✅ `InvestigationPage` muestra un banner amarillo cuando `evidence.detected_os` discrepa de `case.os_profile`, con copy explicando que el cambio lo hace el operador (no Agentopsy). El header del caso también muestra `detectado <os> / <kind>` cuando hay señal. **⚠ Superseded por el enrutado automático (Fase 2 backend hecha); migración de prompts pendiente (pase dedicado) y UI en Fase 3.** |
