...
| `cases/<case-id>/evidence/<id>/` | `forensia.evidence` (`EvidenceManager`) | Hash gate estricto: stream SHA-256 de la fuente → copia (`shutil.copy2`) → re-hash de la copia (abort + cleanup si mismatch) → `chmod 0o444` → `baseline.json`. La fuente nunca se modifica; el handle devuelto apunta SIEMPRE a la copia inmutable. **Registro ATÓMICO**: todo eso se construye en `evidence/.registrando-<eid>` (oculto — su nombre no es un UUID4, así que `list()` jamás lo ve) y se **publica con un único `os.rename`** a `evidence/<eid>`; ante cualquier excepción se borra el temporal, de modo que una interrupción a mitad de copia (504 del proxy, crash, Ctrl-C) no deja un directorio truncado sin `baseline.json`. Progreso OPCIONAL y **observacional** (`on_progress`, ver `evidence_jobs`): no altera el gate. |
| `cases/<case-id>/evidence/.registrando-<id>/` | `forensia.evidence` | Directorio TEMPORAL de un registro en curso (no es evidencia). Se renombra al publicar o se borra al fallar. Si un `kill -9` deja uno huérfano, es seguro borrarlo a mano: nunca fue evidencia registrada. |
...
| `POST`  | `/api/cases/{case_id}/delete`                                   | borra el caso ENTERO (`{confirm_name}` debe ser EXACTAMENTE `case.name`, RULE 2). Irreversible: se lleva evidencia, `audit.jsonl`, hallazgos, artefactos, chats e informes. `409` si la confirmación no cuadra o si el directorio resolvió fuera de la raíz de casos (SEC INV 6) — sin borrar nada. No deja entrada de audit: el log vive dentro de lo que se borra (queda en el log del servidor) |
| `POST`  | `/api/cases/{case_id}/evidence`                                 | registra evidencia (`{source_path}`) de forma SÍNCRONA; ejecuta el hash gate; rechaza con 422 si el caso está `closed`. Se mantiene para MCP / scripts / tests — para una imagen grande usa el asíncrono |
| `POST`  | `/api/cases/{case_id}/evidence/async`                           | arranca el MISMO registro en segundo plano (`forensia.evidence_jobs`) y devuelve el job al instante (`{job_id, state, …}`) — es lo que evita el 504 del proxy en imágenes de decenas de GB. `404` caso inexistente, `422` `source_path` vacío; el resto de guardas (caso cerrado, ruta inexistente, set EWF incompleto) llegan como `error` accionable del job |
| `GET`   | `/api/cases/{case_id}/evidence/jobs`                            | jobs de registro del caso (recientes primero) — la web los consulta al montar para re-enganchar el sondeo de uno vivo |
| `GET`   | `/api/cases/{case_id}/evidence/jobs/{job_id}`                   | estado del job: `pending`/`running` (con `phase ∈ {hashing, copying, verifying}`, `seg_index/seg_count`, `bytes_done/bytes_total` = 3× el tamaño del conjunto), `done` (con `evidence_id`) o `error` (mensaje accionable). `404` si el job no existe o es de otro caso |
