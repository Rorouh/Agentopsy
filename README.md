...
- **Las conclusiones abren su fuente.** Cada hallazgo cita la ejecución, el artefacto y el punto exacto (líneas, bytes o un registro con nombre) que lo sostiene, y desde la interfaz se abre esa cita: se ve el extracto real y si su integridad sigue en pie. Una fuente alterada no enseña extracto, dice qué le pasa.
- **Aprobar un informe es un acto comprobado.** Antes de marcarlo como final se verifica su integridad, la cadena de auditoría, cada fuente citada y las limitaciones que el caso obliga a declarar; el perito aprueba una revisión concreta, identificada por su hash. Es una aprobación humana auditada, no una firma digital criptográfica, y la aplicación lo dice así. Un informe o una fuente alterados bloquean la aprobación y el PDF sale marcado como borrador.
