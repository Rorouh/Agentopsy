...
A diferencia de `python -m agentopsy.server` (modo debug en el host: puerto
...
(ver agentopsy/routers/session.py).

`AGENTOPSY_UI_ORIGINS` (coma-separado) declara los orígenes exactos de la UI
...
def main() -> None:
    ui_origins = tuple(
        origin.strip()
        for origin in os.environ.get("AGENTOPSY_UI_ORIGINS", "").split(",")
        if origin.strip()
    )
