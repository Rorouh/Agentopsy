...
compose — RULE 1 — and holds no keys — RULE 7).

**The graph refreshes itself.** A `SessionStart` hook in `.claude/settings.json` runs
[`tools/graph/graph-refresh.py`](tools/graph/graph-refresh.py) before the session reads
anything: it fingerprints `backend/` + `web/`, and rebuilds `tools/graph/out/` through
the platform runner ONLY when the code actually moved (a no-op costs ~1s). It also
writes `tools/graph/out/GRAPH_STATUS.md` — which commit the graph corresponds to, how
big it is, and **whether `CONTEXT.md` is anchored to an older commit**. Read that card
before trusting `CONTEXT.md`'s numbers. What the hook deliberately does NOT do is
rewrite `CONTEXT.md`: that file is the map the team curates, so after a large
structural change you still update it by hand and re-anchor the commit (RULE 4). Full
guide and guardrails: [`tools/graph/README.md`](tools/graph/README.md).
